...
bool DynamicArray<T>::add(const T &new_entry)
{
    try
    {
        ...
        *************************************/
        ...
        items_[item_count_++] = new_entry; // add item to the end
        return true;
    }
    catch (...)
    {
...
bool DynamicArray<T>::remove(const T &an_item)
{
    try
    {
        ...
        *************************************/
        ...
        // replace the found index with the last index and decrease item count
...
void DynamicArray<T>::resize()
{
    ...
    *************************************/
    ...
    // check if items was cleared before
    if (items_ == nullptr)
    {
        items_ = new T[current_capacity_];
    }

    if (atCapacity() || item_count_ < current_capacity_ / 4)
    {        
        if (atCapacity()) // full and need to expand
        {
            current_capacity_ *= 2;
        }
        else if (current_capacity_ > 1) // sparse and need to shrink
        {
            current_capacity_ /= 2;
        }

        T *new_items = new T[current_capacity_]; // create a new array
        // copy values
